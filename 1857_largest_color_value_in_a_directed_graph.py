...
class Solution:
    def largestPathValue(self, colors: str, edges: List[List[int]]) -> int:
        m = defaultdict(set)
        res = [{} for _ in range(len(colors))]
        incoming = [0] * len(colors)

        for u,v in edges:
            m[u].add(v)
            incoming[v] += 1
        stack = [i for i in range(len(colors)) if incoming[i] == 0]

        while stack:
            u = stack.pop()
            if colors[u] in res[u]:
                res[u][colors[u]] += 1
            else:
                res[u][colors[u]] = 1
        ...
        return -1 if sum(incoming) > 0 else max([max(i.values()) for i in res])

# best solution, good use of map to copy the values
class Solution:
    def largestPathValue(self, colors: str, edges: List[List[int]]) -> int:
